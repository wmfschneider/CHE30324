...
#print(fmin,fmax)
#kfcolors = (fcolors - fmin)/(fmax - fmin)
...
*** Many-electron problem, Schr\ouml{}dinger equation not solvable in closed form (Sad!)
...
1. Schr\ouml{}dinger equation for atom with nuclear charge $Z$:
...
2. Electron-electron repulsion terms $1/| {\bf r_i} - {\bf r_j} |$ are problematic
3. Many approaches, most popular and powerful based on electron density $\rho$
   \[\rho({\bf r})_1 = \int \cdot \int |\Psi({\bf r}_1, {\bf r}_2,\ldots)|^2 d{\bf r}_2 \cdots d{\bf r}_n\]
4. Kohn (1960's) shows that one-electron wavefunctions \(\psi_i \) can be rigorously reintroduced
    \[ \rho({\bf r}) = \sum_i n_i \psi_i^2({\bf r})\]
5. Expression for \(\psi_i \) becomes:
   \[ \left \{ -\frac{\hbar^2}{2m_e}\nabla^2 - \frac{Z}{r} + v_\text{ee}(\rho({\bf r})) \right \}\psi_i = \epsilon_i \psi_i \]
6. Basis of /density functional theory/
   1. Name derives from fact that \(v_\text{ee}\) is a function of the /function/ \(\rho\)
   2. Solutions include one-electron \(psi_i\) and \(\epsilon_i\), total energy \(E\), and of course \(\rho\)! 
...
*** Electron-electron interaction potential \(v_\text{ee}\) contributions
   1. Coulomb ($\hat v_\mathrm{Coul}(\rho)$): classical repulsion between distinguishable electron "clouds"
   2. Exchange ($\hat v_\mathrm{ex}(\rho)$): accounts for electron indistinguishability (Pauli principle for fermions).  Decreases Coulomb repulsion because electrons of like spin intrinsically avoid one another
   3. Correlation ($\hat v_\mathrm{corr}(\rho)$): decrease in Coulomb repulsion due to dynamic ability of electrons to avoid one another; "fixes" orbital approximation
...
3. Modern (DFT): Include Coulomb and approximations to exchange and correlation         
*** Self-consistent field
1. All potential terms $\hat v$ depend on \(\rho\), which is unknown /a priori/, so  equations must be solved /iteratively/ to /self-consistency/
   1. Guess \(\rho\)
   2. Construct \(v_\text{ee}\)
   3. Set up and solve DFT equation for \(\psi_i\)
   4. Get new \(\rho\) from \(\psi_i\)
   5. Compute \(|\rho_\text{new} - \rho_\text{old}| \)
      1. If less than tolerance, done!
      2. If not, return to step 2
2. Solved on a grid or by expanding \(\psi_i\) in a basis
*** DFT calculations on atoms
1. Original [[https://doi.org/10.1017/S0305004100011919][Hartree paper]]
2. See [[../Resources/fda/00README.txt][README]] at [[../Resources/fda]]
...
*** Math to construct molecular orbitals
1. Express wavefunctions ("molecular orbitals") as linear combination of atomic-like orbitals
	\begin{equation}
          \psi_\mathrm{MO}=\sum_a c_a\phi_a({\bf r})
	\end{equation}
2. Find set of $c_a$ that give best wavefunctions (MOs)
...
1. Only AOs of appropriate symmetry, overlap, and energy match can combine to form MOs
	...
	\epsilon_-\approx \alpha_2 - \beta S + \beta^2/|\alpha_1-\alpha_2|
      \end{eqnarray*}
2. LiH: H 1s + Li 2s, $\sigma$ bond polarized towards H
   1. So much so, that LiH forms an ionic solid
3. BH: H 1s and Li 2p$_z, $\sigma$ bond polarized towards H
   1. "hybridize" Li 2s and Li 2p to get /three/ H 1s + Li sp$^2$ bonds, yields trigonal \ce{BH3}
4. CH: same ideas lead to tetrahedral \ce{CH4}, gas
5. HF: H 1s + F 2p, bond polarized towards F, lots of non-bonding orbitals
...
*** COMMENT The H\uuml{}ckel/tight binding model: [[http://resolver.caltech.edu/CaltechBOOK:1961.001][Roberts, Notes on Molecular Orbital Theory]]
